...
def parse_args():
  ...
  parser.add_argument('-i', '--input_dir', type=str, required=True,
                      help='Directory to the inputs. (required)')
  parser.add_argument('-n', '--chosen_num_or_ratio', type=float, default=0.4,
...
def main():
  """Main function."""
  args = parse_args()
  logger = setup_logger(args.input_dir, logger_name='generate_data')

  logger.info('Loading latent codes.')
  latent_codes_path = os.path.join(args.input_dir, 'z.npy')
  ...
  scores_path = os.path.join(args.input_dir, f'{args.attribute}_scores.npy')
  ...
  boundary = train_boundary(latent_codes=latent_codes,
                            ...
                            logger=logger)
  np.save(f'boundaries/pggan_celebahq_{args.attribute}_boundary.npy', boundary)
